    ...
    request(options, (err, res) => {
        ...
        if (response.hasOwnProperty('graphql')) {
            ...
            if (type === 'GraphImage') {
                ...
                metadata.video = { thumbnail: response.graphql.shortcode_media.display_url, videoUrl: response.graphql.shortcode_media.video_url};
                ...
                response.graphql.shortcode_media.edge_sidecar_to_children.edges.map((item) => {
                    ...
                    if (item.node.__typename == 'GraphVideo') metadata.video.push({ thumbnail: item.node.display_url, videoUrl: item.node.video_url});
                });
            };
            return metadata;
        } else {
            return new Error('User is Private or Post not Found')
        }
    });
}